...
interface LikedRoomsModalProps {
  isOpen: boolean;
  onClose: () => void;
  likedRooms: Room[];
  likedRoomIds: string[];
...
export default function LikedRoomsModal({
  isOpen,
  onClose,
  likedRooms,
  likedRoomIds,
    ...
    return createPortal(
      <div className="fixed inset-0 z-[200] flex items-center justify-center p-4">
        ...
        <div className="relative w-full max-w-md z-10 animate-fade-in bg-white rounded-[32px] shadow-2xl p-8 text-center border border-white/80">
          ...
          <h3 className="text-xl font-bold text-slate-800 mb-2">Chưa có phòng trọ yêu thích</h3>
          <p className="text-slate-500 mb-6">
            Bạn chưa lưu phòng trọ nào. Hãy thả tym để lưu lại nhé!
          </p>
          <button
            onClick={onClose}
      ...
      <div className="relative w-full max-w-7xl z-10 animate-fade-in">
        ...
        <div className="bg-[#fbfdff] rounded-[32px] shadow-2xl border border-white/80 w-full max-h-[90vh] flex flex-col overflow-hidden">
          ...
          <section className="relative overflow-hidden border-b border-sky-100 bg-white shadow-[0_4px_20px_rgba(15,80,110,0.05)] shrink-0">
            <div className="absolute inset-x-0 top-0 h-full bg-gradient-to-br from-[#f0f9ff] via-[#e0f2fe] to-[#bae6fd] opacity-40" />
            <div className="absolute -left-8 top-5 h-32 w-32 rounded-full bg-sky-400/20 blur-2xl" />
            <div className="absolute right-12 top-0 h-28 w-28 rounded-full bg-blue-400/15 blur-2xl" />
            ...
            <div className="relative flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4 p-6 sm:p-8 pr-14">
              <div className="flex items-center gap-4">
                <div className="relative h-14 w-14 overflow-hidden rounded-2xl border-2 border-white bg-white shadow-md flex items-center justify-center shrink-0">
                  ...
                  <Building className="w-7 h-7 text-sky-500 relative z-10" />
                </div>
                <div>
                  <h2 className="text-2xl sm:text-3xl font-black text-[#07132d] tracking-tight">Phòng Trọ Yêu Thích</h2>
                  <p className="text-slate-500 text-sm mt-1 font-medium">Tất cả những phòng trọ mà bạn đã lưu lại</p>
                </div>
              </div>
              <div className="inline-flex items-center gap-2 px-4 py-2 rounded-full bg-white border border-sky-100 text-sky-600 font-bold text-sm shadow-sm backdrop-blur-md">
                <Heart className="w-4 h-4 fill-current text-sky-500" />
                {likedRooms.length} phòng yêu thích
              </div>
            </div>
          </section>
